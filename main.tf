...
locals {
  ...
  abbreviated_load_balancer_name = join(
    "",
    [
      substr(
        local.desired_load_balancer_name,
        0,
        length(local.desired_load_balancer_name) >= 24 ? 24 : length(local.desired_load_balancer_name),
      ),
...
module "alb" {
  ...
  name = length(local.desired_load_balancer_name) > 32 ? local.abbreviated_load_balancer_name : local.desired_load_balancer_name
...
locals {
  ...
  abbreviated_default_target_group_name = join(
    "",
    [
      substr(
        local.desired_default_target_group_name,
        0,
        length(local.desired_default_target_group_name) >= 24 ? 24 : length(local.desired_default_target_group_name),
      ),
...
resource "aws_alb_target_group" "default_target_group" {
  name = length(local.desired_default_target_group_name) > 32 ? local.abbreviated_default_target_group_name : local.desired_default_target_group_name
